...
public class GUIMachineCustom extends GuiInfoContainer {
	...
	private static final ResourceLocation texture = new ResourceLocation(Tags.MODID + ":textures/gui/processing/gui_custom.png");
		...
		if(this.mc.player.inventory.getItemStack().isEmpty()) {
			for(int i = 0; i < this.inventorySlots.inventorySlots.size(); ++i) {
				...
				if(this.isMouseOverSlot(slot, x, y) && slot instanceof SlotPattern && custom.matcher.modes[tileIndex - 10] != null) {
					this.drawHoveringText(Arrays.asList(TextFormatting.RED + "Right click to change", ModulePatternMatcher.getLabel(custom.matcher.modes[tileIndex - 10])), x, y - 30);
				}
			}
		}
	...
	protected void drawGuiContainerForegroundLayer(int i, int j) {
		...
		this.fontRenderer.drawString(name, 68 - this.fontRenderer.getStringWidth(name) / 2, 6, 4210752);
		this.fontRenderer.drawString(I18n.format("container.inventory"), 8, this.ySize - 96 + 2, 4210752);
		if(custom.config.fluxMode) this.fontRenderer.drawString("Flux:" + custom.flux, 83, 57, 0x08FF00);
	}
