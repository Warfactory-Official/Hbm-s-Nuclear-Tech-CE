...
import com.hbm.util.FontRendererUtil;
...
public class GuiScreenAUTOCAL extends GuiScreen {
    protected static final ResourceLocation texture = new ResourceLocation(Tags.MODID, "textures/gui/machine/gui_rtty_autocal.png");
    ...
    public GuiScreenAUTOCAL(TileEntityRadioAUTOCAL autocal) {
    ...
    protected void mouseClicked(int x, int y, int i) throws IOException {
        ...
        if(checkClick(x, y, 144, 36, 18, 18)) {
            try {
                ...
                if(!doc.exists()) {
                    doc.createNewFile();
                    try {
                        PrintWriter printer = new PrintWriter(doc, StandardCharsets.US_ASCII);
        ...
        if(data != null) {
            ...
            PacketThreading.createSendToServerThreadedPacket(new NBTControlPacket(data, autocal.getPos()));
        }
    }
    ...
    private void drawGuiContainerForegroundLayer(int x, int y) {
        ...
        if(checkClick(x, y, 84, 36, 18, 18)) this.drawHoveringText(TextFormatting.BLUE + "Upload Program", x, y);
