...
public class ContainerSoyuzCapsule extends Container {
	...
	public ContainerSoyuzCapsule(InventoryPlayer invPlayer, TileEntitySoyuzCapsule tedf) {
		...
		for(int i = 0; i < 3; i++)
		{
			for(int j = 0; j < 6; j++)
			{
				this.addSlotToContainer(new SlotItemHandler(tedf.inventory, j + i * 6, 26 + j * 18 + 18 * 2, 18 + i * 18));
			}
		}

		this.addSlotToContainer(new SlotItemHandler(tedf.inventory, 18, 17, 36));
