...
import static com.hbm.inventory.recipes.ChemicalPlantRecipes.INSTANCE;

/**
 * Dedicated GroovyScript integration for the Chemical Plant (com.hbm.inventory.recipes.ChemicalPlantRecipes).
 * Exposed as mods.hbm.chemicalPlant. The name is explicit (not derived from this class's simple name)
 * because the recipeOverrides system already auto-registers a RecipeFileBinding under the alias
 * "chemicalplant" (all lowercase) for this same machine - using the class-name-derived default would
 * silently collide with, and lose to, that existing binding.
 */
@RegistryDescription(linkGenerator = "hbm", isFullyDocumented = false)
public class ChemicalPlant extends VirtualizedRegistry<GenericRecipe> {

    public ChemicalPlant() {
        super(Collections.singletonList("chemicalPlant"));
    }
    ...
    // ==================== Debug / verification ====================
    // See AssemblyMachine's twin implementation for the rationale (not shared on purpose).
    ...
    public void debugDump() {
        GroovyLog.get().infoMC("[hbm] chemicalPlant: {} total recipes ({} stock/loaded, {} added via groovy this session)",
    ...
    public static class RecipeBuilder extends AbstractRecipeBuilder<GenericRecipe> {
        ...
        public String getRecipeNamePrefix() {
            return "groovyscript_chemicalplant_";
        }

        /**
         * The Chemical Plant uses hbm's own FluidStack type, not Forge's - so unlike items, fluids
         * can't go through the base builder's fluidInput()/fluidOutput() (Forge-typed, unused here).
         * Construct with e.g. new FluidStack(Fluids.WATER, 2000). Up to 3 each way.
         */
        ...
        public String getErrorMsg() {
            return "Error adding Chemical Plant recipe";
        }
        ...
        public void validate(GroovyLog.Msg msg) {
            ...
            if (this.fluidsIn.length > 3) {
                msg.add("{} input fluids given, but the Chemical Plant allows at most 3", this.fluidsIn.length);
            }
            if (this.fluidsOut.length > 3) {
                msg.add("{} output fluids given, but the Chemical Plant allows at most 3", this.fluidsOut.length);
            }
            if (this.input.isEmpty() && this.fluidsIn.length == 0) {
                msg.add("Chemical Plant recipe needs at least one item or fluid input");
            }
            if (this.output.isEmpty() && this.fluidsOut.length == 0) {
                msg.add("Chemical Plant recipe needs at least one item or fluid output");
            }
        }
        ...
        public GenericRecipe register() {
            ...
            HbmGroovyPropertyContainer.CHEMICALPLANT.addRecipe(recipe);
            return recipe;
        }
    }
}
