...
import static com.hbm.inventory.recipes.RockMillRecipes.INSTANCE;

/**
 * Dedicated GroovyScript integration for the Rock Mill (com.hbm.inventory.recipes.RockMillRecipes).
 * Exposed as mods.hbm.rockMill - explicit name to avoid the recipeOverrides RecipeFileBinding alias
 * "rockmill" (see AssemblyMachine's javadoc for the full rationale).
 */
@RegistryDescription(linkGenerator = "hbm", isFullyDocumented = false)
public class RockMill extends VirtualizedRegistry<GenericRecipe> {

    public RockMill() {
        super(Collections.singletonList("rockMill"));
    }
    ...
    public void debugDump() {
        GroovyLog.get().infoMC("[hbm] rockMill: {} total recipes ({} stock/loaded, {} added via groovy this session)",
    ...
    public static class RecipeBuilder extends AbstractRecipeBuilder<GenericRecipe> {
        ...
        public String getErrorMsg() {
            return "Error adding Rock Mill recipe";
        }
        ...
        public void validate(GroovyLog.Msg msg) {
            ...
            if (this.input.isEmpty() && this.fluidIn == null) {
                msg.add("Rock Mill recipe needs at least one item or fluid input");
            }
            if (this.output.isEmpty() && this.fluidOut == null) {
                msg.add("Rock Mill recipe needs at least one item or fluid output");
            }
        }
        ...
        public GenericRecipe register() {
            ...
            HbmGroovyPropertyContainer.ROCKMILL.addRecipe(recipe);
            return recipe;
        }
    }
}
