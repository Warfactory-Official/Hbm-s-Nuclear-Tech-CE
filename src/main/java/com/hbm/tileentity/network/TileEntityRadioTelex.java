...
public class TileEntityRadioTelex extends TileEntityLoadedBase implements IControlReceiver, IGUIProvider, SimpleComponent, CompatHandler.OCComponent, ITickable {
    ...
    public void update() {
        if(!world.isRemote) {
                ...
                if(sendingWait > 0) {
                    ...
                    if(line.length() > sendingIndex) {
                        ...
                        if(c == pause) {
                            ...
                            RTTYSystem.broadcast(world, this.txChannel, c);
                            this.sendingChar = c;
                        }
                    } else {
                        ...
                        if(sendingLine >= 4) {
                            ...
                            RTTYSystem.broadcast(world, this.txChannel, eot);
                            this.sendingLine = 0;
                        } else {
                            RTTYSystem.broadcast(world, this.txChannel, eol);
                            this.sendingLine++;
                        }
                        this.sendingIndex = 0;
                    }
            ...
            if(!this.rxChannel.isEmpty()) {
                RTTYSystem.RTTYChannel chan = RTTYSystem.listen(world, this.rxChannel);

                if(chan != null && chan.signal instanceof Character && (chan.timeStamp > world.getTotalWorldTime() - 2 && chan.timeStamp != -1)) {
                    ...
                    if(c == eot) {
                        ...
                        world.playSound(null, pos, SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP, SoundCategory.BLOCKS, 2.0F, 0.5F);
                    } else if(c == print) {
    ...
    public boolean hasPermission(EntityPlayer player) {
        return player.getDistanceSq(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5) < 16 * 16;
    }
