...
public abstract class TileEntityTurretBaseNT extends TileEntityMachineBase implements IEnergyReceiverMK2, IControllable, IControlReceiver, ITickable, SimpleComponent, IRORInteractive, CompatHandler.OCComponent {
        ...
        if (!world.isRemote) {
            ...
            if (isOn() && hasPower()) {
                ...
                if (searchTimer <= 0) {
                    ...
                    if (this.target == null && !manualOverride) this.seekNewTarget();
                }
            } else {
                searchTimer = 0;
            }
    ...
    protected void seekNewTarget() {
        ...
        List<Entity> allEntities = world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(pos.x, pos.y, pos.z, pos.x, pos.y, pos.z).grow(range, range, range));
        ...
        if (target != null) this.tPos = this.getEntityPos(this.target);
    }
    ...
    public boolean entityAcceptableTarget(Entity e) {
        ...
        if (targetMobs) {
            ...
            if (e instanceof MultiPartEntityPart) return true;
    ...
    public Vec3d byHorizontalIndexOffset() {
        ...
        if (meta == 2) return new Vec3d(1, 0, 1);
        if (meta == 4) return new Vec3d(1, 0, 0);
        if (meta == 5) return new Vec3d(0, 0, 1);
    ...
    public Object[] addWhitelist(Context context, Arguments args) {
        if (this.getWhitelist() != null) {
            ...
            if (names.contains(args.checkString(0))) return new Object[]{false};
        }
