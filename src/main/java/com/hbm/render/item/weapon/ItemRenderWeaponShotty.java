		...
		case FIRST_PERSON_RIGHT_HAND:
			if(GeneralConfig.ssgAnim) {
				if(type == TransformType.FIRST_PERSON_LEFT_HAND) {
					...
					GlStateManager.scale(1, 1.5, 1.5);
				} else {
				...
				if(entity.isSneaking()) {
					...
					GlStateManager.rotate((float) (-eject[2] * 0.25), 0, 0, 1);
				}

				GlStateManager.translate(-recoil[0] * 2, 0, 0);
				GlStateManager.rotate((float) (recoil[0] * 5), 0, 0, 1);
				
				GlStateManager.pushMatrix();
				GlStateManager.rotate((float) (-eject[2] * 0.8), 0, 0, 1);
				ResourceManager.shotty.renderPart("Barrel");
				
				GlStateManager.pushMatrix();
				GlStateManager.rotate((float) (ejectShell[0] * 90), 0, 0, 1);
