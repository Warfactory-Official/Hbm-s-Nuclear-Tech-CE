...
@AutoRegister(tileentity = TileEntityTurretRichard.class)
public class RenderTurretRichard extends RenderTurretBase<TileEntityTurretRichard>
  ...
  public void render(
    ...
    GlStateManager.translate(0, 0.375, 0.1875);

    for (int i = 0; i < turret.loaded; i++) {
      ResourceManager.turret_richard.renderPart("MissileLoaded");

      if (i == 2 || i == 6 || i == 9 || i == 13) {
        GlStateManager.translate(0, -0.1875, 0.1875 * 2 + 0.09375);
      } else {

        GlStateManager.translate(0, 0, -0.1875);
      }
    }
