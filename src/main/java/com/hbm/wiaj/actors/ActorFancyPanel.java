...
public class ActorFancyPanel implements ISpecialActor {
	...
	@Override public void tick(JarScene scene) { }
	...
	protected void drawGradientRect(int minX, int minY, int maxX, int maxY, int color1, int color2) {
		...
		bufferBuilder.pos(maxX, minY, zLevel).color(r1, g1, b1, a1).endVertex();
		bufferBuilder.pos(minX, minY, zLevel).color(r1, g1, b1, a1).endVertex();
		bufferBuilder.pos(minX, maxY, zLevel).color(r2, g2, b2, a2).endVertex();
	...
	public void drawTexturedModalRect(int posX, int posY, int u, int v, int sizeX, int sizeY, int color) {
		...
		BufferBuilder bufferBuilder = tessellator.getBuffer();
		bufferBuilder.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_COLOR);
		bufferBuilder.pos(posX + 0, posY + sizeY, zLevel).tex((u + 0) * f, (v + sizeY) * f1).color(r, g, b, a).endVertex();
		bufferBuilder.pos(posX + sizeX, posY + sizeY, zLevel).tex((u + sizeX) * f, (v + sizeY) * f1).color(r, g, b, a).endVertex();
		bufferBuilder.pos(posX + sizeX, posY + 0, zLevel).tex((u + sizeX) * f, (v + 0) * f1).color(r, g, b, a).endVertex();
		bufferBuilder.pos(posX + 0, posY + 0, zLevel).tex((u + 0) * f, (v + 0) * f1).color(r, g, b, a).endVertex();
		tessellator.draw();
	}
